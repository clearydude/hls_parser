...
fn parse_tag_name(tag_str: &str) -> nom::IResult<&str, &str> {
    take_until(":")(tag_str)
}
...
fn parse_variant_stream(variant_stream_str: &str) -> nom::IResult<&str, TagWithURI> {
    map(
        separated_pair(parse_tag_and_attributes, tag("\n"), parse_value),
        |((name, attributes), uri)| TagWithURI {
            name: name.to_string(),
            attributes,
            uri,
        },
...
fn parse_tag(tag_str: &str) -> nom::IResult<&str, Tag> {}
